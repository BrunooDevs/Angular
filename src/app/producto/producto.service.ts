...
export class ProductoService {
   ...
   private uriEndPoint:string = "http://localhost:8080/api/productos"
  ...
  reactivo */
    ...
    */
     return this.http.get<producto[]>(this.uriEndPoint).pipe(
       ...
       map(response => {
         ...
         let clientes = response as producto[];
   ...
   create(cliente: producto) : Observable<any>{
  ...
  //metodo que edita un cliente
   getCliente( id: number) : Observable<producto> {
     ...
     return this.http.get<producto>( `${this.uriEndPoint}/${id}`).pipe(
   ...
   Observable<any> ya que retornamos un Hasmap desde el backen y con any le decimos que puede ser cualquier tipo de datos que se recibira, de esta forma obtenemos los mensajes directamente de la respuesta   
   */
   update(cliente: producto) : Observable<any> {
     return this.http.put<any>(`${this.uriEndPoint}/${cliente.nproducto}`,cliente, {headers: this.httpHeaders}).pipe(
   ...
   delete (id: number) : Observable<producto>{
     return this.http.delete<producto>(`${this.uriEndPoint}/${id}`, {headers: this.httpHeaders}).pipe(
