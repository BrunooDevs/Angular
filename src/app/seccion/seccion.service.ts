...
export class SeccionService {
   ...
   private uriEndPoint:string = "http://localhost:8080/api/secciones"
  ...
  reactivo */
   getClientes():Observable <seccion[]>{
    ...
    */
     return this.http.get<seccion[]>(this.uriEndPoint).pipe(
       ...
       map(response => {
         ...
         let seccion = response as seccion[];
         ...
         return seccion.map(seccion => {
             seccion.nombre =seccion.nombre?.toUpperCase();
   ...
   create(cliente: seccion) : Observable<any>{
  ...
  //metodo que edita un cliente
   getCliente( id: number) : Observable<seccion> {
     /*stream de interpolacion  `` soncomillas invertidas agregamos la variable uriEndPoint*/
     return this.http.get<seccion>( `${this.uriEndPoint}/${id}`).pipe(
   ...
   Observable<any> ya que retornamos un Hasmap desde el backen y con any le decimos que puede ser cualquier tipo de datos que se recibira, de esta forma obtenemos los mensajes directamente de la respuesta   
   */
   update(seccion: seccion) : Observable<any> {
     return this.http.put<any>(`${this.uriEndPoint}/${seccion.nseccion}`,seccion, {headers: this.httpHeaders}).pipe(
   ...
   delete (id: number) : Observable<seccion>{
     return this.http.delete<seccion>(`${this.uriEndPoint}/${id}`, {headers: this.httpHeaders}).pipe(
