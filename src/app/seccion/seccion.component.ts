...
export class SeccionComponent implements OnInit{
  ...
  ngOnInit() {
    this.seccionService.getClientes().subscribe(
  ...
  delete (secciones: seccion) : void{
    ...
    swalWithBootstrapButtons.fire({
      title: 'Estas seguro?',
      text: `Seguro que deseas eliminar al cliente ${secciones.nombre}  ${secciones.edo} ? `,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        this.seccionService.delete(secciones.nseccion).subscribe(
          response => {
            ...
            cli => cli !== cliente: indica que si cli de cliente es distinto al cliente que vamos a eliminar si es distinto lo vamos a mostrar en la lista si no lo va a quitar de la lista al cliente eliminado*/
              this.seccion = this.seccion?.filter(cli => cli !== secciones);
          }
        ...
        swalWithBootstrapButtons.fire(
          ...
          'seccoin eliminado!',
          `seccion ${secciones.nombre} eliminado con exito`,
          'success'
        )
