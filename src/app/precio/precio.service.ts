...
export class PrecioService {
   ...
   private uriEndPoint:string = "http://localhost:8080/api/precios"
  ...
  reactivo */
    ...
    */
     return this.http.get<precio[]>(this.uriEndPoint).pipe(
       ...
       map(response => {
         ...
         let clientes = response as precio[];
   ...
   create(cliente: precio) : Observable<any>{
  ...
  //metodo que edita un cliente
   getCliente( id: number) : Observable<precio> {
     /*stream de interpolacion  `` soncomillas invertidas agregamos la variable uriEndPoint*/
     return this.http.get<precio>( `${this.uriEndPoint}/${id}`).pipe(
   ...
   Observable<any> ya que retornamos un Hasmap desde el backen y con any le decimos que puede ser cualquier tipo de datos que se recibira, de esta forma obtenemos los mensajes directamente de la respuesta   
   */
   update(cliente: precio) : Observable<any> {
     return this.http.put<any>(`${this.uriEndPoint}/${cliente.nprecio}`,cliente, {headers: this.httpHeaders}).pipe(
   ...
   delete (id: number) : Observable<precio>{
     return this.http.delete<precio>(`${this.uriEndPoint}/${id}`, {headers: this.httpHeaders}).pipe(
